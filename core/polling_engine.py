...
class MockS7Connection:
    ...
    Cabin types (configurable):
      ...
      - LEAK:      pressure starts ~634, drops ~0.45 mbar/point
    ...
    def db_read(self, db_number: int, start: int, size: int) -> bytearray:
        ...
        for i in range(self._cabin_count):
            ...
            if ct == "NO_BOTTLE":
                ...
                pressure = self._pressure_bases[i] + self.LEAK_SLOPE * cp + random.gauss(0, self.LEAK_NOISE)
            else:  # OK
                pressure = self._pressure_bases[i] + self.OK_SLOPE * cp + random.gauss(0, self.OK_NOISE)
            ...
            buf += bytearray(2)                # +12: Bool flags
            buf += struct.pack(">f", 0.0)      # +14: cabinHealthStatus
            buf += bytearray(2)                # +18: leakValveStatus + pad
